...
**Причина**: Повреждение `db.sqlite3` или неверные права доступа к директории `fsp/`
**Решение**:
```bash
# Проверка наличия файла БД и прав
ls -la fsp/db.sqlite3

# Применение миграций (создаст/обновит БД)
cd fsp
python manage.py migrate

# Если файл БД поврежден: пересоздать
rm -f db.sqlite3
python manage.py migrate
```
